...
map("n", "<leader>gs", "<cmd>Git status<CR>", opts "Search Git status")
...
map(
  "n",
  "<leader>fz",
  ":lua require('telescope').extensions.live_grep_args.live_grep_args({search_dirs={vim.fn.expand(\"%\")}})<CR>",
  opts "Live grep current buffer"
)
...
map("n", "<leader>fr", "<cmd>Telescope resume<CR>", opts "Last Tele history")
...
map(
  "v",
  "<C-r>",
  'y<ESC><cmd>lua require(\'telescope\').extensions.live_grep_args.live_grep_args({search_dirs={vim.fn.expand("%")}})<CR><C-R>"',
  { remap = true }
)
-- :cdo %s//g<left
